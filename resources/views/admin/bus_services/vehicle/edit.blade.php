...
@section('content')
    <div class="card shadow">
        <div class="card-body">
            <form action="{{ route('admin.busServices.vehicle.edit', $Vehicle->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-4">
                        <div>
                            ...
                            <input type="file" name="image_1" class="form-control dropify" accept=".jpg,.jpeg,.png"
                                id="image" data-default-file="{{asset($Vehicle->image_1)}}">
                        </div>
                        <hr>
                            ...
                            @for ($index = 2; $index <= 7; $index++)
                                <div class="col-md-6 mb-2">
                                    ...
                                    <input type="file" name="image_{{ $index }}" class="form-control  dropify"
                                        ...
                                        data-default-file="{{ asset($Vehicle->{'image_'.$index})}}">
                                </div>
                            @endfor
                        </div>
                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="bus_type_id" class="form-control" id="bus_type_id" required>
                                        ...
                                        @foreach ($busTypes as $type)
                                            <option value="{{ $type->id }}" {{ $Vehicle->bus_type_id == $type->id ? 'selected' : '' }}>
                                                {{ $type->bus_type_name }}
                                            </option>
                                        @endforeach
                                    </select>
